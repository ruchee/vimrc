...
" Based on the Java indentation rules by Toby Allsopp <toby.allsopp@peace.com> (resigned)
...
" The "extends", "satisfies" and "given" lines start off with the wrong indent.
setlocal indentkeys& indentkeys+=0=extends indentkeys+=0=satisfies indentkeys+=0=given

" Set the function to do the work.
setlocal indentexpr=GetCeylonIndent()

let b:undo_indent = "set cin< cino< indentkeys< indentexpr<"

" Only define the function once.
if exists("*GetCeylonIndent")
  finish
endif

function! SkipCeylonBlanksAndComment(startline)
...
function GetCeylonIndent()
  ...
  let lnum = SkipCeylonBlanksAndComment(v:lnum - 1)
  let prev = lnum
  while prev > 1
    let next_prev = SkipCeylonBlanksAndComment(prev - 1)
  ...
  " classes.
  if getline(v:lnum) =~ '^\s*\(extends\|satisfies\|given\)\>'
        \ && getline(lnum) !~ '^\s*\(extends\|satisfies\|given\)\>'
  ...
  let cont_kw = matchstr(getline(prev),
        \ '^\s*\zs\(throws\|satisfies\|given\|extends\)\>\ze.*,\s*$')
  if strlen(cont_kw) > 0
  ...
  elseif getline(prev) =~ '^\s*\(throws\|satisfies\|given\|extends\)\>'
        \ && (getline(prev) =~ '{\s*$'
        \  || getline(v:lnum) =~ '^\s*{\s*$')
    let theIndent = theIndent - &sw
  endif
  ...
  if getline(v:lnum) =~ '^\s*}\s*\(//.*\|/\*.*\)\=$'
    ...
    if lnum < v:lnum
      while lnum > 1
        let next_lnum = SkipCeylonBlanksAndComment(lnum - 1)
        if getline(lnum) !~ '^\s*\(throws\|extends\|given\|satisfies\)\>'
              \ && getline(next_lnum) !~ ',\s*$'
          break
        endif
