...
" [-- count indent-increasing spacebars tags of line a:lnum --]
fun! <SID>SpacebarsIndentOpen(lnum)
...
" [-- count indent-decreasing spacebars tags of line a:lnum --]
fun! <SID>SpacebarsIndentClose(lnum)
  let S = 'x'.getline(a:lnum)
    let s = substitute('x'.getline(a:lnum),
    \ '.\{-}\(\({{/\)\([^}]*\)\>}}\)', "\1", 'g')
    let s = substitute(s, "[^\1].*$", '', '')
    return strlen(s)
...
fun! <SID>HtmlIndentSum(lnum, style)
    if a:style == match(getline(a:lnum), '^\s*\(<\|{{\)/')
        if a:style == match(getline(a:lnum), '^\s*\(</\<\('.s:html_indent_tags.'\)\>\|{{/\)')
            ...
            let open = open + <SID>SpacebarsIndentOpen(a:lnum)
            let close = <SID>HtmlIndentClose(a:lnum, s:html_indent_tags)
            let close = close + <SID>SpacebarsIndentClose(a:lnum)
            if 0 != open || 0 != close
                return open - close
            endif
        endif
